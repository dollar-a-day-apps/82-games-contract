...
contract Game82 is BaseAdmin {
  ...
  event Predict(uint256 log_id, address player, uint16  game_id, uint16  points, uint16  rebounds, uint16  assists);

  address public addrFinance;              // the address to hold the funds from player purchases
  uint256 public voucherPrice = 100000;    // the rate/price of 1 voucher against 'sun'
  ...
  // Unpack the prediction metadata based on the diagram below:
  // 0x   04      03      02      01
  //   assists rebounds points  gameId
  //            string index:
  //       0       1       2       3
  function _getPredictParams(bytes _extraData) private pure
    returns (uint16 game_id, uint16 points, uint16 rebounds, uint16 assists) {
    game_id = uint16(_extraData[3]);
    points = uint16(_extraData[2]);
    rebounds = uint16(_extraData[1]);
    assists = uint16(_extraData[0]);
  }

  // Handle the actual prediction validation and recording
  function submitPrediction(uint256 _value, bytes _extraData) external {
    // Check if the user has enough voucher
    uint256 voucherCount = tokenContract.balanceOf(msg.sender);
    require(voucherCount >= _value);
    require(_value == 1 && _extraData.length >= 4);
    ...
    // Parse the prediction metada
    // Then proceed with burning the vouchers consumed/used
    (game_id, points, rebounds, assists) = _getPredictParams(_extraData);
    require(game_id >= 1);
    require(tokenContract.burn(msg.sender, _value));

    uint256 logId = predictArray.length;
    PredictInfo memory pi = PredictInfo(uint160(msg.sender), game_id, points, rebounds, assists);
    ...
    emit Predict(logId, msg.sender, game_id, points, rebounds, assists);
  }
